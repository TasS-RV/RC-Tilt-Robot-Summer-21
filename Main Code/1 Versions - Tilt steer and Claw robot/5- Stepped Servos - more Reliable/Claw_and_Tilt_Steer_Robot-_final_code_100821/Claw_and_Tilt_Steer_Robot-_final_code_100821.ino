/*Program written by Tasin Sayed. Date: 10 August 2021 for final draft.
...
if (current_timeH - previous_timeH >15){  //Maybe use 35 
  previous_timeH = current_timeH;
  
if (repeater <= 4){ //Allows a multiple of 3 iterations of the servo write: 'rec' packet not updated then servo motion will stop after 3 iterations  
   
   if (N_ > 700){
    Hservo.detach();Nservo.attach(6);
    Npos+=30;
   if (Npos > 2000){Npos = 2000;}
   Nservo.write(Npos);
}
...
wdt_disable(); //Prevents unnecessary resets for low probability events
 ...
 if (switch_state == 0){
    switch_state = 1; Serial.println("Drive Resetting"); 
