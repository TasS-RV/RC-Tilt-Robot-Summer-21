...
//Issue: motor drivers stop functioning after first swithc  back to 'drive' mode - required to reset the Arduino
...
if (mode == "D"){
  ...
  if (switch_state == 0){
    ...
    sleep_enable();
    delay(50); sleep_disable();
    
   // resetFunc;    
   /*Sleep and Reset functions both did not work - the watchdog timer did not work either, therefore rather 
   than a soft reset mode, we need to activate a hardware reset or something that clears the registers. and
   clears all pinModes - the pinModes are likely the key issue, due to interference with Servo pins.
   
   100% issue: associated with detaching and attaching the servo pins - if they are not attache din the first place
   switching back to drive mode does not affect motor*/
    }
